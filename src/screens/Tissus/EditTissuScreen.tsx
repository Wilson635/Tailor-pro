// ==========================================
// ÉCRAN MODIFICATION TISSU — TailorPro (Module 6)
// ==========================================
...
// ÉCRAN PRINCIPAL
// ==========================================
export const EditTissuScreen: React.FC<Props> = ({ route, navigation }) => {
  const insets = useSafeAreaInsets();
  const { tissuId } = route.params;

  const { getTissuById, updateTissu } = useAppStore();
  const tissu = getTissuById(tissuId);

  if (!tissu) {
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
        <Text style={{ color: C.textSec }}>Tissu introuvable</Text>
  ...
  const [typeTissu,       setTypeTissu]       = useState(tissu.typeTissu);
  const [nomCommercial,   setNomCommercial]   = useState(tissu.nomCommercial);
  const [couleur,         setCouleur]         = useState(tissu.couleur);
  const [fournisseur,     setFournisseur]     = useState(tissu.fournisseur ?? '');
  const [prixUnitaire,    setPrixUnitaire]    = useState(tissu.prixUnitaire.toString());
  const [quantiteUtilisee,setQuantiteUtilisee]= useState(tissu.quantiteUtilisee.toString());
  const [photoUri,        setPhotoUri]        = useState<string | undefined>(tissu.photo);
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Modifier le tissu</Text>
        <View style={{ width: 36 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.photoRow}>
            {photoUri ? (
              ...
              <TouchableOpacity style={styles.addPhotoBtn} onPress={pickPhoto}>
                ...
                <Text style={styles.addPhotoText}>Changer la photo</Text>
              </TouchableOpacity>
            )}
          </View>
        </Section>
        ...
        <Section title="Informations">
          <Field label="Nom commercial">
            <Inp value={nomCommercial} onChangeText={setNomCommercial} placeholder="Nom commercial…" />
          </Field>
          <Field label="Couleur">
            <Inp value={couleur} onChangeText={setCouleur} placeholder="Couleur…" />
          ...
          <Field label="Fournisseur">
            <Inp value={fournisseur} onChangeText={setFournisseur} placeholder="Fournisseur…" />
          </Field>
        </Section>
        ...
        <Section title="Prix & quantité">
          <Field label="Prix unitaire (F/m)">
            <Inp value={prixUnitaire} onChangeText={setPrixUnitaire} keyboardType="decimal-pad" />
          </Field>
          <Field label="Quantité utilisée (m)">
            <Inp value={quantiteUtilisee} onChangeText={setQuantiteUtilisee} keyboardType="decimal-pad" />
          </Field>
        </Section>
...
const styles = StyleSheet.create({
  ...
  addPhotoBtn:  { width: 120, height: 120, borderRadius: 14, borderWidth: 1.5, borderColor: C.border,
    borderStyle: 'dashed', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: C.surface },
  addPhotoText: { fontSize: 11, color: C.textSec, textAlign: 'center' },
